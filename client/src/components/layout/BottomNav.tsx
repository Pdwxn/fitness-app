...
export function BottomNav({ items, sidebarTitle }: BottomNavProps) {
  ...
  return (
    <>
      <nav aria-label="Main navigation" className="fixed bottom-4 left-4 right-4 z-50 flex h-20 items-center justify-around rounded-[2rem] border border-white/15 bg-black/80 px-2 pb-[env(safe-area-inset-bottom)] shadow-[0_-18px_50px_rgba(0,0,0,0.45)] backdrop-blur-xl md:hidden">
        {items.map((item) => {
      ...
      <aside className="hidden md:fixed md:left-0 md:top-0 md:flex md:h-screen md:w-64 md:flex-col md:border-r md:border-white/10 md:bg-black/80 md:p-5 md:backdrop-blur-xl">
        ...
        <nav aria-label="Sidebar navigation" className="mt-6 flex flex-col gap-2">
          {items.map((item) => {
            const isActive = pathname === item.href;
            return (
              <Link
